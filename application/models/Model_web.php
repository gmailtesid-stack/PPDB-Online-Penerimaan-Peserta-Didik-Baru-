...
class Model_web extends CI_Model
{
    ...
    function pendaftaran($menu = '', $data = '', $berkas = array())
    {
        switch ($menu) {
            case 'daftar':
                $data = (object) $data;
                ...
                if (strpos($message, 'duplicate entry') !== false && strpos($message, 'primary') !== false) {
                    ...
                    $needs_manual_id = true;
                } elseif (strpos($message, 'duplicate entry') !== false && strpos($message, 'id_siswa') !== false) {
                    $needs_manual_id = true;
                }
            ...
            case 'id_baru':
                ...
                // INI BAGIAN YANG DIBENERIN BIAR GAK ERROR NULL
                $kode_max = isset($no_max->kode) ? $no_max->kode : '';
                $no_urut = (int) substr((string) $kode_max, 6) + 1;
            ...
            case 'v_pekerjaan_wali':
                return $this->db->select('id_pekerjaan, nama_pekerjaan')->group_by('id_pekerjaan, nama_pekerjaan')->order_by('id_pekerjaan', 'ASC')->get('tbl_pekerjaan')->result();
                break;
